...
相较于 ``model.py`` 中的基线模型，本模型：

* 将卷积通道扩展为 16 -> 32 -> 64；
* 使用 3x3、padding=1 的卷积保留输入空间尺寸；
* 在每个卷积后使用 BatchNorm2d 和可学习时间常数的 PLIF；
* 仅在第二、第三个卷积块后下采样；
* 使用全局平均池化，避免参数集中在全连接分类头；
* 直接对逐时间步的模拟分类电流取平均作为 logits，避免最终输出
  脉冲计数的量化与并列问题。
"""
...
class ConvSNNv2(nn.Module):
    ...
    第三个（最后一个）隐藏 PLIF 的发放率兼容别名。
    """

    EXPECTED_PARAMETER_COUNT = 25_686
    ...
    def _make_plif(self) -> neuron.ParametricLIFNode:
        """创建具有可学习时间常数的 PLIF 神经元。"""
    ...
    def forward_sequence(
        ...
        Returns:
            ...
            ``firing_rates`` 保存三个隐藏 PLIF 的平均发放率。
        """
